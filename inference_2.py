...
def main():
    load_dotenv()
    original_test = pd.read_csv('data/test_with_rag_ranked_filtered_1k_1024.csv')
    model_name = "microsoft/phi-2"
    finetunedFolder = "Koleshjr/phi-2-teleqa-1-0-0-5-epochs-lr-0-001-full-dataset-1024-1k"
    ...
    if os.path.exists('data/test_with_rag_ranked_filtered_progress_1k_5epochs_1k_1024_model.csv'):
        shutil.copy('data/test_with_rag_ranked_filtered_progress_1k_5epochs_1k_1024_model.csv',
                    'data/test_with_rag_ranked_filtered_progress_backup_1k_5epochs_1k_1024_model.csv')
        mod_test = pd.read_csv('data/test_with_rag_ranked_filtered_progress_1k_5epochs_1k_1024_model.csv', on_bad_lines='skip', engine='python')
    else:
    ...
    for i, row in tqdm(null_responses.iterrows(), total=null_responses.shape[0]):
        ...
        with torch.no_grad():
            ...
            test.to_csv('data/test_with_rag_ranked_filtered_progress_1k_5epochs_1k_1024_model.csv', index=False)
    ...
    submission.to_csv('data/submission_with_rag_ranked_filtered_progress_1k_5epochs_1k_1024_model.csv', index=False)
